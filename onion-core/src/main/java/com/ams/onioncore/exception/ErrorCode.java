...
public enum ErrorCode {
    ...
    CANNOT_ADD_SELF(HttpStatus.BAD_REQUEST, "자기 자신은 추가할 수 없습니다."),
    ALREADY_FRIENDS(HttpStatus.CONFLICT, "이미 친구 목록에 존재합니다."),
    ALREADY_REQUESTED(HttpStatus.CONFLICT, "이미 요청을 보냈습니다."),
    ...
    ALREADY_PARTY_MEMBER(HttpStatus.CONFLICT, "이미 파티에 가입한 사용자입니다."),
    INVALID_JOIN_TYPE(HttpStatus.CONFLICT, "해당 파티는 요청 방식이 맞지 않습니다."),
    ...
    CANNOT_KICK_CREATOR(HttpStatus.FORBIDDEN, "파티 생성자는 강퇴할 수 없습니다."),

    NOT_PARTY_MEMBER(HttpStatus.BAD_REQUEST, "해당 사용자는 파티 멤버가 아닙니다."),
    CREATOR_CANNOT_LEAVE(HttpStatus.BAD_REQUEST, "방장은 파티를 나갈 수 없습니다."),
    CANNOT_KICK_LEADER(HttpStatus.BAD_REQUEST, "리더는 강퇴할 수 없습니다."),
    USER_MUTED(HttpStatus.FORBIDDEN, "해당 파티에서 음소거 상태입니다.");
